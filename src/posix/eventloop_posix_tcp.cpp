...
TCP_setNoNagle(UA_FD sockfd) {
    int val = 1;
    //int res = UA_setsockopt(sockfd, NSAPI_SOCKET, TCP_NODELAY, &val, sizeof(val));
    int res = 0;
    if(res < 0)
...
TCP_listenSocketCallback(UA_ConnectionManager *cm, TCP_FD *conn, short event) {
    ...
    UA_FD newsockfd = (UA_FD)UA_accept(conn->rfd.fd, &remote);
    ...
    int get_res = UA_getnameinfo(&remote, hoststr);
    if(get_res != 0) {
        UA_LOG_SOCKET_ERRNO_WRAP(
...
static UA_StatusCode
TCP_registerListenSocket(UA_POSIXConnectionManager *pcm, SocketAddress *ai,
    ...
    char hoststr[UA_MAXHOSTNAME_LENGTH];
    int get_res = UA_getnameinfo(ai, hoststr);
    if(get_res != 0) {
        hoststr[0] = 0;
        UA_LOG_SOCKET_ERRNO_WRAP(
           UA_LOG_WARNING(el->eventLoop.logger, UA_LOGCATEGORY_NETWORK,
    ...
    UA_FD listenSocket = (UA_FD)socket(0, 0, 0);
    ...
    int optval = 1;
#if UA_IPV6
    if(ai->get_ip_version() == NSAPI_IPv6) {
        UA_LOG_WARNING(el->eventLoop.logger, UA_LOGCATEGORY_NETWORK,
...
TCP_openActiveConnection(UA_POSIXConnectionManager *pcm, const UA_KeyValueMap *params,
    ...
    SocketAddress hints, *info;
    int error = NetworkInterface::get_default_instance()->getaddrinfo(hostname, &hints, &info);
    if(error != 0) {
        UA_LOG_WARNING(el->eventLoop.logger, UA_LOGCATEGORY_NETWORK,
    ...
    /* Create a socket */
    UA_FD newSock = (UA_FD)socket(0, 0, 0);
